...
def setup(args):
    ...
    model = VisionTransformer(config, args.img_size, zero_head=True, num_classes=22, smoothing_value=args.smoothing_value)
    pretrained_model = torch.load('autodl-tmp/ba/ALSO_DEN_checkpoint.bin')['model']
...
def process_video(args, model):
    ...
    # Prepare a video writer to save the output video
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    out = cv2.VideoWriter(os.path.join(args.output_dir, 'output_video.avi'), fourcc, 30.0, (args.img_size, args.img_size))
    ...
    while cap.isOpened():
        ...
        img = torch.tensor(img).permute(2, 0, 1).unsqueeze(0).to(args.device)  # Change to C x H x W format

        # Make prediction
        with torch.no_grad():
            outputs = model(img)
            logits = outputs[0] if isinstance(outputs, tuple) else outputs  # Extract logits from tuple
            preds = torch.argmax(logits, dim=-1).cpu().numpy()[0]  # Get predicted class

        # Get class name from the mapping
        class_name = class_mapping.get(preds, "Unknown Class")  # Default to "Unknown Class" if not found

        # Add the predicted label to the frame
        label_text = f"Predicted: {class_name}"
        cv2.putText(frame, label_text, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

        # Write the frame to output video
        out.write(frame)

        # Optionally display the frame (comment this out if you don't want to display)
...
def main():
    ...
    parser.add_argument("--video_path", type=str, default="shark_fina.mp4", help="Path to the video file for testing.")
