...
#     pretrained_mohttps://storage.googleapis.com/vit_models/imagenet21k/https://storage.googleapis.com/vit_models/imagenet21k/del = torch.load('autodl-tmp/ba/cubb_checkpoint.bin')['model']
...
def train(args, model):
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss_fct = torch.nn.CrossEntropyLoss()
            logits1, logits2,logits3 = model(x)
            # print(f"logits1 shape: {logits1.shape}, y shape: {y.shape}")
            ...
            train_loss1 = loss_fct(logits1, y)
            train_loss1 = train_loss1.mean()
            train_loss2 = loss_fct(logits2, y)
            train_loss2 = train_loss2.mean()
            train_loss3 = loss_fct(logits3, y)
            train_loss3 = train_loss3.mean()

            
            loss = (train_loss1+train_loss2+train_loss3)/3
...
def main():
    ...
    parser.add_argument("--dataset", choices=["ASLO", "SHARK","WILDFISH"], default="ASLO",
                        help="Which dataset.")
    ...
    parser.add_argument("--output_dir", default="autodl-tmp/ba", type=str,
